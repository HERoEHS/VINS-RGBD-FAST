...
//
// ★종료(정착) 판정의 실체(07-14 실측): 양자화 엔코더라 순간 변화율은 '0 또는
//   ≥LSB/샘플주기(≈1.7rad/s)'의 이진 신호 — 스윙 중에도 0.05~1.0rad/s 구간 샘플은 0개.
//   따라서 rate_min은 (0, 1.7) 구간에서 불감(연속값 엔코더 대비용 일반화 파라미터)이고,
...
public:
    struct Params
    {
        ...
        double rate_min     = 0.05;  // [rad/s] '아직 움직임' 판정(종료용). 양자화 스파이크(≈1.7)보다 낮으면 충분 — (0,1.7) 구간 불감
